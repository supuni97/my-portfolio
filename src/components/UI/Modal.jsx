...
const portfolio = portfolios.find(portfolio=>portfolio.id===activeID)
  ...
  return (
    ...
    <div className='max-w-[600px] absolute top-1/2 left-1/2 z-20 bg-white rounded-[8px] transform -translate-x-1/2 -translate-y-1/2 p-5'>
      ...
      <h4 className='text-[18px] text-headingColor text-[700]'>Technologies:</h4>
      {
        portfolio.technologies.map((item,index)=>(
          <span key={index} className='py-1 px-2 bg-gray-200 rounded-[5px] text-[14px] leading-0'>{item}</span>
        ))
      }
      </div>
      ...
      <button className='w-[1.8rem] h-[1.8rem] bg-white absolute top-[1.7rem] right-[1.7rem] text-[25px] flex items-center justify-center rounded-[3px] leading-0 cursor-pointer' onClick={()=>setShowModal(false)}>&times;</button>
    </div>  
    </div>
  )
}
